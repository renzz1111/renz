...
async function startSychBot() {
    ...
    const waitingMessage = () => {
        // Cek jika loading sudah ditampilkan, agar tidak spam
        if (isLoadingDisplayed) return;

        isLoadingDisplayed = true; // Tampilkan loading pertama kali

        setInterval(() => {
            dots += '.';
            if (dots.length > 3) dots = ''; // Reset titik setelah 3 titik
            process.stdout.write(chalk.yellow(`Menunggu pesan${dots}   \r`));  // Update pada baris yang sama
        }, 500); // Interval 500ms (setiap setengah detik)
    };

    waitingMessage(); // Memulai animasi menunggu pesan...
